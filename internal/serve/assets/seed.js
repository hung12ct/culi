...
const SEED = {
  ...
  analytics: {
    available: true,
    range: 'Last 7 days',
    summary: { activeCards: 5, sessions: 3, deliveries: 13, tokens: 2692, crossHarnessCards: 2, helpful: 2, noisy: 1, expensive: 1, uncertain: 1 },
    repos: [
    ...
    cards: [
      { id: 'rules/go-error-wrapping', short: 'a3f1', title: 'Wrap errors with %w, never %v', type: 'rule', available: true, sessions: 3, deliveries: 4, tokens: 460, lastUsed: 'today 10:53', bucket: 'helpful', pullRate: '29%', repos: ['phin', 'cutpilot'], harnesses: [
      ...
      { id: 'skills/git-worktree', short: 'b710', title: 'Set up a git worktree for parallel work', type: 'skill', available: true, sessions: 2, deliveries: 3, tokens: 820, lastUsed: 'today 09:22', bucket: 'noisy', pullRate: '9%', repos: ['phin'], harnesses: [
      ...
      { id: 'styles/commit-subject', short: 'c8b2', title: 'Commit subjects: imperative, ≤50 chars', type: 'style', available: true, sessions: 2, deliveries: 2, tokens: 288, lastUsed: 'yesterday 16:41', bucket: 'helpful', pullRate: '28%', repos: ['phin'], harnesses: [
        { code: 'claude', label: 'Claude Code', sessions: 2, deliveries: 2, tokens: 288 }] },
      { id: 'patterns/debounce-cursor', short: 'd4e9', title: 'Debounce cursor events at 16ms', type: 'pattern', available: true, sessions: 1, deliveries: 2, tokens: 404, lastUsed: 'today 09:22', bucket: 'uncertain', pullRate: '', repos: ['cutpilot'], harnesses: [
        { code: 'codex', label: 'Codex CLI', sessions: 1, deliveries: 2, tokens: 404 }] },
      { id: 'rules/removed-card', short: '', title: 'rules/removed-card', type: 'unknown', available: false, sessions: 1, deliveries: 2, tokens: 720, lastUsed: 'yesterday 16:30', bucket: 'expensive', pullRate: '', repos: ['phin'], harnesses: [
  ...
  cards: (function () {
    ...
    const EFF_NOTE = {
      helpful: 'Pulled or referenced after injection — this card earns its tokens.',
      noisy: 'Downvoted or repeatedly ignored as a pointer. Consider retiring or tightening its triggers.',
      expensive: 'High token spend with no observed pull. Full-card pushes can still help invisibly — judge with the body granularity in mind.',
      uncertain: 'Not enough observations to judge yet.',
    };
    const eff = b => ({ bucket: b, note: EFF_NOTE[b] });
    const sp = () => [55, 70, 40, 85, 60, 75, 50];
    return [
      { id: 'a3f1', type: 'rule', title: 'Wrap errors with %w, never %v', status: 'confirmed', scopeLabels: ['lang:go'], baseline: true, injected: 214, key: 'go/errors', source: 'import', mergedFrom: 'phin, cutpilot', model: 'claude-sonnet', hash: '9f3c…a1', spark: sp(), triggers: ['error', 'wrap', 'errors.Is', 'fmt.Errorf', 'lỗi'], usage: u('214', '61', '18', '0'), eff: eff('helpful'),
      ...
      { id: 'c8b2', type: 'style', title: 'Commit subjects: imperative, ≤50 chars', status: 'confirmed', scopeLabels: ['global'], baseline: true, injected: 188, key: 'git/commit', source: 'manual', mergedFrom: '—', model: '—', hash: '2c4d…9b', spark: sp(), triggers: ['commit', 'message', 'git'], usage: u('188', '40', '12', '2'), eff: eff('helpful'),
      ...
      { id: 'd4e9', type: 'pattern', title: 'Debounce cursor events at 16ms', status: 'candidate', scopeLabels: ['repo:cutpilot'], baseline: false, injected: 3, key: '', source: 'learn', mergedFrom: '—', model: 'claude-haiku', hash: 'aa90…c2', spark: sp(), triggers: ['debounce', 'cursor', 'raf'], usage: u('3', '1', '0', '0'), eff: eff('helpful'),
      ...
      { id: 'b710', type: 'skill', title: 'Set up a git worktree for parallel work', status: 'confirmed', scopeLabels: ['global'], baseline: false, injected: 96, key: 'git/worktree', source: 'gen', mergedFrom: '—', model: 'claude-sonnet', hash: '55ab…7d', spark: sp(), triggers: ['worktree', 'branch', 'parallel'], usage: u('96', '2', '0', '9'), eff: eff('noisy'),
      ...
      { id: '6f2c', type: 'agent', title: 'Reviewer agent: security-focused pass', status: 'confirmed', scopeLabels: ['repo:phin'], baseline: false, injected: 41, key: 'agent/review', source: 'mcp', mergedFrom: '—', model: 'claude-sonnet', hash: 'cd12…4e', spark: sp(), triggers: ['review', 'security', 'audit'], usage: u('41', '20', '11', '1'), eff: eff('helpful'),
      ...
      { id: '9d01', type: 'lesson', title: 'py2-compat shims no longer needed', status: 'retired', scopeLabels: ['lang:python'], baseline: false, injected: 0, key: '', source: 'import', mergedFrom: '—', model: '—', hash: '77ce…10', spark: [10, 8, 6, 4, 2, 0, 0], triggers: ['python2', 'compat', '__future__'], usage: u('0', '0', '0', '6'), eff: eff('noisy'),
      ...
      { id: '3a55', type: 'rule', title: 'Never log secrets, even at debug level', status: 'confirmed', scopeLabels: ['global'], baseline: true, injected: 176, key: 'sec/logging', source: 'manual', mergedFrom: '—', model: '—', hash: '88fa…22', spark: sp(), triggers: ['log', 'secret', 'token', 'redact'], usage: u('176', '33', '9', '0'), eff: eff('helpful'),
      ...
      { id: '1e88', type: 'style', title: 'Prefer early returns over nested ifs', status: 'confirmed', scopeLabels: ['lang:go', 'lang:ts'], baseline: false, injected: 122, key: '', source: 'gen', mergedFrom: '—', model: 'claude-sonnet', hash: '0a0a…33', spark: sp(), triggers: ['guard', 'early return', 'nesting'], usage: u('122', '14', '5', '1'), eff: eff('helpful'),
